...
# For example, the list [(4,7),(1,2)] means that the first reservation is from the day 4 to the day 7, and the second reservation is from the day 1 to the day 2. These two reservations do not ovelap. On the other hand, the reservations on the list [(4,7),(1,5)] do overlap, because both ranges contain the days 4 and 5.
...
def check_overlapping(reservations):
    ...
    sorted_res = sorted(reservations, key=lambda r: r[0])

    max_end = sorted_res[0][1]

    for i in range(1, len(sorted_res)):
        start, end = sorted_res[i]
        if start <= max_end:
            return True
