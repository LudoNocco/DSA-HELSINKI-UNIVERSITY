class Robot:
    def __init__(self, input_string, rules):
        self.tape = ["L"] + list(input_string) + ["R"]
        self.head = 0
        self.state = 1
        self.steps = 0
        self.rule_dict = {}
        for (sym, st, nsym, nst, act) in rules:
            self.rule_dict[(sym, st)] = (nsym, nst, act)

    def step(self):
        if self.head < 0 or self.head >= len(self.tape):
            return "REJECT"
        key = (self.tape[self.head], self.state)
        if key not in self.rule_dict:
            return "REJECT"
        nsym, nst, act = self.rule_dict[key]
        self.tape[self.head] = nsym
        self.state = nst
        if act == "LEFT":
            self.head -= 1
        elif act == "RIGHT":
...
def calculate(input_string, rules):
    robot = Robot(input_string, rules)
    while robot.steps < 1000:
        robot.steps += 1
        action = robot.step()
        if action == "ACCEPT":
            return True
