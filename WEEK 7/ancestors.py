...
def ancestor(node, a, b):
    def trova_percorso(n, x, percorso):
        if n.value == x:
            return percorso + [n.value]
        for figlio in n.children:
            risultato = trova_percorso(figlio, x, percorso + [n.value])
            if risultato:
                return risultato
        return None
    cammino = trova_percorso(node, b, [])
    return cammino is not None and a in cammino
